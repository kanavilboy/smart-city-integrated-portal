...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("SELECT id FROM gas_customers WHERE consumer_no = ?");
    $stmt->execute(array($consumer_no));
    if ($stmt->fetch()) {
        $errors['consumer_no'] = 'Consumer number already exists';
    }
    ...
    if (empty($errors)) {
        try {
            $stmt = $conn->prepare("
                ...
                (consumer_no, dealer_id, full_name, phone, address, city, state, pincode) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ");
...
<head>
    ...
    <title>Add Customer - Gas Agency</title>
    <!-- BOOTSTRAP STYLES-->
            ...
            <div class="adjust-nav">
                <div class="navbar-header">
                    ...
                    <a class="navbar-brand" href="#">
                        <?php echo htmlspecialchars($agency_name); ?> - Add Customer
                    </a>
                </div>
            ...
            <div id="page-inner">
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <h5>Add a new customer to your gas agency</h5>
                    </div>
                </div>
                <!-- /. ROW  -->
                <hr />
